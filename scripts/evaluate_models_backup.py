...
Evaluation Tasks:
    ...
    - Task 2: Fill-in-the-Blank / Virtual Sensing (causal chain validation)
    - Task 3: FDD - Fault Detection & Diagnostics (physical consistency)
    - Extra: OOD Stress Test (extreme condition generalization)
...
# =============================================================================
# FDD Scenarios Configuration - Task 3: Fault Detection & Diagnostics
# =============================================================================
# FDD tests physical causal chain learning:
#   Weather -> ODU Power -> IDU Power -> Zone Temps
#
# Each scenario: given context variables, reconstruct target variables
# If model learned physics, reconstruction should be accurate
FDD_SCENARIOS = {
    'FDD-ODU': {
        'target_ids': list(range(12, 14)),      # ODU Power
        'context_ids': list(range(0, 8)) + list(range(50, 62)),  # Weather + Zone Temps
        'description': 'Given Weather+Zone -> Reconstruct ODU Power',
        'causal_chain': 'Weather -> ODU'
    },
    'FDD-IDU': {
        'target_ids': list(range(30, 34)),      # IDU Power
        'context_ids': list(range(0, 8)) + list(range(12, 14)) + list(range(50, 62)),
        'description': 'Given Weather+ODU+Zone -> Reconstruct IDU Power',
        'causal_chain': 'ODU -> IDU'
    },
    'FDD-Zone': {
        'target_ids': list(range(50, 62)),      # Zone Temps
        'context_ids': list(range(0, 8)) + list(range(12, 14)) + list(range(30, 34)),
        'description': 'Given Weather+ODU+IDU -> Reconstruct Zone Temps',
        'causal_chain': 'Power -> Zone'
    },
}
...
def calculate_weighted_quantile_loss(
    ...
    """
    Weighted Quantile Loss (wQL) - Industry standard metric.

    Used by DeepAR, Prophet, Amazon Forecast for probabilistic forecasting.

    Formula:
        wQL_q = 2 * sum(QuantileLoss_t) / sum(|y_true|)

    Advantages:
        1. Self-weighted: High power moments matter more, standby fluctuations matter less
        2. No division by zero: denominator is sum(|y_true|), only fails if all zeros
        3. Interpretable: wQL=0.1 means ~10% error relative to total magnitude

    Args:
        y_true: Ground truth values (T,)
        samples: Samples from predictive distribution (N, T)
        quantiles: Quantiles to evaluate

    Returns:
        Dict with wQL for each quantile and wQL_mean
    """
    y_true = y_true.flatten()
    mask = ~np.isnan(y_true)
    y_true = y_true[mask]
    ...
    if len(y_true) < 10:
        return {f'wQL_{int(q*100)}': np.nan for q in quantiles}

    total_abs_y = np.sum(np.abs(y_true))

    # Conservative threshold: IDU fan 0.1kW is normal, don't misclassify
    # Only return nan for truly all-zero data
    if total_abs_y < 1e-10:
        return {f'wQL_{int(q*100)}': np.nan for q in quantiles}

    results = {}
    for q in quantiles:
        q_pred = np.percentile(samples, q * 100, axis=0)

        # Pinball loss (quantile loss)
        error = y_true - q_pred
        pinball = np.where(error >= 0, q * error, (q - 1) * error)

        # Weighted: divide by total magnitude, not mean
        wql = 2 * np.sum(pinball) / total_abs_y
        results[f'wQL_{int(q*100)}'] = wql

    # Summary metric: mean of all quantile losses
    results['wQL_mean'] = np.mean([results[f'wQL_{int(q*100)}'] for q in quantiles])
...
def calculate_msis(
    ...
    # Scale by seasonal error with conservative relative threshold
    # IDU fan 0.1kW is normal, ODU standby 0.1kW is normal - don't misclassify
    min_seasonal_error = max(
        1e-6,  # Absolute floor (very conservative)
        0.01 * np.std(y_true) if len(y_true) > 0 else 1e-6,  # 1% of target std
    )
    effective_seasonal_error = max(seasonal_error, min_seasonal_error)
    msis = score / effective_seasonal_error
...
def calculate_all_metrics(
    ...
    if samples is not None and len(samples) > 1:
        ...
        # Weighted Quantile Loss (wQL) - industry standard
        wql_results = calculate_weighted_quantile_loss(y_true, samples)
        metrics.update(wql_results)
...
def calculate_fdd_metrics(
    ...
    """
    Calculate FDD-specific metrics for reconstruction quality.

    FDD (Fault Detection & Diagnostics) tests whether the model learned
    physical causal chains. If learned correctly, the model can accurately
    reconstruct a target variable given other physically-related variables.

    Metrics:
        - Recon_MAE: Reconstruction Mean Absolute Error
        - Recon_SMAPE: Reconstruction Symmetric MAPE
        - Mean_Z: Mean Z-score (if samples available)
        - Max_Z: Maximum Z-score (peak anomaly)
        - Anomaly_Rate: % of points where |Z| > 3 (3-sigma rule)

    Args:
        y_true: Ground truth values (T,)
        y_pred: Reconstructed values (T,)
        samples: (N, T) samples from predictive distribution (optional)
    ...
    # Reconstruction MAE
    metrics['Recon_MAE'] = np.mean(np.abs(y_true_clean - y_pred_clean))

    # Reconstruction SMAPE
    denom = np.abs(y_true_clean) + np.abs(y_pred_clean) + 1e-8
    metrics['Recon_SMAPE'] = 200 * np.mean(np.abs(y_true_clean - y_pred_clean) / denom)

    # Z-score metrics (if samples available for uncertainty estimation)
    if samples is not None and samples.shape[0] > 1:
        samples_clean = samples[:, mask]
        y_std = np.std(samples_clean, axis=0)
        y_std = np.maximum(y_std, 1e-6)  # Avoid division by zero

        z_scores = np.abs(y_true_clean - y_pred_clean) / y_std

        metrics['Mean_Z'] = np.mean(z_scores)
        metrics['Max_Z'] = np.max(z_scores)
        metrics['Anomaly_Rate'] = np.mean(z_scores > 3.0) * 100  # 3-sigma rule, as percentage
    else:
...
def prepare_multivariate_input(
    ...
    # Context boundary in patches (do NOT include pad in the boundary to avoid leaking future)
    context_patches = (context_length + patch_size - 1) // patch_size
    context_end = pad_amount + context_length  # timestep index (with pad) where context ends
    ...
    # For virtual_sensor task: mask middle 30% of target vars (time-based, not patch-count based)
    vs_mask_start_idx = int(total_length * 0.35)
    vs_mask_end_idx = int(total_length * 0.65)
        ...
        for p in range(num_patches_per_var):
            ...
            if task_type == 'forecast':
                ...
                if var_id in weather_var_ids:
                    # Weather always visible (history + future)
                    observed_mask[0, patch_idx, :patch_size] = ~nan_mask[start_idx:end_idx]
                    prediction_mask[0, patch_idx] = False
                else:
                    if patch_end <= context_end:
                        # Entire patch in context
                        observed_mask[0, patch_idx, :patch_size] = ~nan_mask[start_idx:end_idx]
                        prediction_mask[0, patch_idx] = False
                    elif patch_start >= context_end:
                        # Entire patch in future
                        observed_mask[0, patch_idx, :patch_size] = False
                        prediction_mask[0, patch_idx] = True
                    else:
                        # Mixed patch: partially context, partially future
                        cutoff = context_end - patch_start
                        cutoff = max(0, min(patch_size, cutoff))
                        observed_mask[0, patch_idx, :cutoff] = ~nan_mask[start_idx:start_idx + cutoff]
                        observed_mask[0, patch_idx, cutoff:patch_size] = False
                        prediction_mask[0, patch_idx] = True

            elif task_type == 'virtual_sensor':
                # Virtual sensor: target vars masked in middle 30% (time-based), others fully visible
                if var_id in target_var_ids:
                    mask_start = vs_mask_start_idx + pad_amount
                    mask_end = vs_mask_end_idx + pad_amount

                    # No overlap -> fully observed
                    if patch_end <= mask_start or patch_start >= mask_end:
                        observed_mask[0, patch_idx, :patch_size] = ~nan_mask[start_idx:end_idx]
                        prediction_mask[0, patch_idx] = False
                    # Fully inside mask -> fully masked
                    elif patch_start >= mask_start and patch_end <= mask_end:
                        observed_mask[0, patch_idx, :patch_size] = False
                        prediction_mask[0, patch_idx] = True
                    else:
                        # Partial overlap: mask the overlapping region, observe the rest
                        m_start_local = max(mask_start, patch_start) - patch_start
                        m_end_local = min(mask_end, patch_end) - patch_start
                        observed_mask[0, patch_idx, :m_start_local] = ~nan_mask[start_idx:start_idx + m_start_local]
                        observed_mask[0, patch_idx, m_start_local:m_end_local] = False
                        observed_mask[0, patch_idx, m_end_local:patch_size] = ~nan_mask[start_idx + m_end_local:end_idx]
                        prediction_mask[0, patch_idx] = True
                else:
...
def task1_standard_forecast(
    ...
    for model_name, metric_list in results.items():
        if len(metric_list) == 0:
            aggregated[model_name] = {k: np.nan for k in ['SMAPE', 'CRPS', 'MSIS', 'MAE', 'SmoothedMAE', 'Coverage', 'wQL_mean']}
        else:
            aggregated[model_name] = {}
            # Collect all keys from all metrics (some may have wQL, some may not)
            all_keys = set()
...
def task2_fill_in_blank(
    ...
    for model_name, metric_list in results.items():
        if len(metric_list) == 0:
            aggregated[model_name] = {k: np.nan for k in ['SMAPE', 'CRPS', 'MSIS', 'MAE', 'wQL_mean']}
        else:
            aggregated[model_name] = {}
            # Collect all keys from all metrics
            all_keys = set()
            for m in metric_list:
                all_keys.update(m.keys())
...
def task3_fdd(
    ...
    """
    Task 3: FDD (Fault Detection & Diagnostics).

    Core idea: If the model learned physical causal chains during training,
    it should be able to accurately reconstruct a target variable given
    other physically-related variables.

    Consistency with training:
        - Training: Random mask variables, model learns P(V1|V2,V3,...,Vn)
        - FDD eval: Systematically mask target variables, test physical chains

    Scenarios tested (from FDD_SCENARIOS):
        - FDD-ODU: Given Weather+Zone -> Reconstruct ODU Power
        - FDD-IDU: Given Weather+ODU+Zone -> Reconstruct IDU Power
        - FDD-Zone: Given Weather+ODU+IDU -> Reconstruct Zone Temps

    Returns:
        Dict[scenario_name][model_name] -> Dict of FDD metrics
    """
    results = {
        scenario: {name: [] for name in models.keys()}
        for scenario in FDD_SCENARIOS.keys()
    }
    ...
    for idx in range(min(len(test_hf), max_samples)):
        ...
        for scenario_name, scenario_config in FDD_SCENARIOS.items():
            target_ids = scenario_config['target_ids']
            context_ids = scenario_config['context_ids']

            # Filter to valid variable IDs (within data range)
            valid_target_ids = [v for v in target_ids if v < num_variates]
            valid_context_ids = [v for v in context_ids if v < num_variates]

            if not valid_target_ids:
                continue

            # Find best window using first target variable
            first_var = valid_target_ids[0]
            var_data = target[first_var, :]
            ...
            # Extract window for ALL variates
            window_data = target[:, start_idx:end_idx].copy()

            # === Baseline: Seasonal Naive ===
            if 'Seasonal Naive' in models and models['Seasonal Naive'] is not None:
                for var_id in valid_target_ids:
                    y_true = window_data[var_id, :]
                    # Seasonal naive: use historical mean as reconstruction baseline
                    sn_pred = np.full_like(y_true, np.nanmean(y_true))
                    metrics = calculate_fdd_metrics(y_true, sn_pred, None)
                    results[scenario_name]['Seasonal Naive'].append(metrics)

            # === MOIRAI models: FDD reconstruction ===
            moirai_models = {k: v for k, v in models.items()
                            if k not in ['Seasonal Naive', 'XGBoost'] and v is not None}

            for model_name, module in moirai_models.items():
                try:
                    # Use predict_fill_in_blank with full mask on target variables
                    # mask_start_ratio=0.0, mask_end_ratio=1.0 means mask entire sequence
                    predictions = predict_fill_in_blank(
                        module,
                        window_data,
                        observed_var_ids=valid_context_ids,
                        masked_var_ids=valid_target_ids,
                        context_length=CONTEXT_LENGTH,
                        prediction_length=PREDICTION_LENGTH,
                        patch_size=PATCH_SIZE,
                        num_samples=NUM_SAMPLES,
                        device=device,
                        mask_start_ratio=0.0,   # Mask from beginning
                        mask_end_ratio=1.0      # Mask to end (full reconstruction)
                    )
                    ...
                    for var_id in valid_target_ids:
                        ...
                        # predict_fill_in_blank returns (point_pred, samples, mask_start, mask_end)
                        result = predictions[var_id]
                        point_pred, samples = result[0], result[1]
                        y_true = window_data[var_id, :]

                        # Align lengths (reconstruction may be shorter due to mask region)
                        min_len = min(len(y_true), len(point_pred))
                        y_true_aligned = y_true[:min_len]
                        point_pred_aligned = point_pred[:min_len]
                        if samples is not None:
                            samples_aligned = samples[:, :min_len]
                        else:
                            samples_aligned = None

                        metrics = calculate_fdd_metrics(y_true_aligned, point_pred_aligned, samples_aligned)
                        results[scenario_name][model_name].append(metrics)

                except Exception as e:
                    print(f"  Warning: FDD {model_name} failed in {scenario_name}: {e}")
                    continue

        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    # Aggregate results: compute mean across samples for each metric
    aggregated = {}
    for scenario_name, scenario_results in results.items():
        aggregated[scenario_name] = {}
        for model_name, metric_list in scenario_results.items():
            if len(metric_list) == 0:
                aggregated[scenario_name][model_name] = {
                    'Recon_MAE': np.nan,
                    'Recon_SMAPE': np.nan,
                    'Mean_Z': np.nan,
                    'Max_Z': np.nan,
                ...
                for key in all_keys:
                    values = [m.get(key, np.nan) for m in metric_list if not np.isnan(m.get(key, np.nan))]
                    aggregated[scenario_name][model_name][key] = np.mean(values) if values else np.nan
...
def task_extra_ood(
    ...
    """
    Extra Task: OOD Stress Test (MULTI-VARIATE).

    NOTE: This is an optional extra test, not core Task 3.
    Task 3 is now FDD (Fault Detection & Diagnostics).
    ...
    for condition, condition_results in results.items():
        ...
        for model_name, metric_list in condition_results.items():
            if len(metric_list) == 0:
                aggregated[condition][model_name] = {k: np.nan for k in ['SMAPE', 'CRPS', 'MSIS', 'MAE', 'wQL_mean']}
            else:
...
def plot_ood_performance(results: Dict[str, Dict[str, Dict[str, float]]], output_dir: Path):
    ...
    plt.suptitle('Extra: OOD Stress Test - SMAPE under Extreme Conditions', fontweight='bold', fontsize=15, y=1.02)
    plt.tight_layout()
...
def plot_fdd_results(results: Dict[str, Dict[str, Dict[str, float]]], output_dir: Path):
    """
    Plot FDD (Fault Detection & Diagnostics) results.

    Creates a bar chart comparing reconstruction accuracy across scenarios
    and models, showing the model's ability to learn physical causal chains.
    """
    scenarios = list(results.keys())
    if not scenarios:
        return

    # Get all model names from first scenario
    models = list(next(iter(results.values())).keys())
    if not models:
        return

    # Create figure: one subplot per scenario
    n_scenarios = len(scenarios)
    fig, axes = plt.subplots(1, n_scenarios, figsize=(8 * n_scenarios, 8))

    if n_scenarios == 1:
        axes = [axes]

    scenario_display = {
        'FDD-ODU': 'FDD: ODU Power\n(Given Weather+Zone)',
        'FDD-IDU': 'FDD: IDU Power\n(Given Weather+ODU+Zone)',
        'FDD-Zone': 'FDD: Zone Temps\n(Given Weather+Powers)',
    }

    for ax_idx, scenario in enumerate(scenarios):
        ax = axes[ax_idx]

        # Use Recon_SMAPE as primary metric
        smape_values = [results[scenario][m].get('Recon_SMAPE', np.nan) for m in models]
        colors = [MODEL_COLORS.get(m, '#95a5a6') for m in models]

        # Filter out NaN values for plotting
        valid_idx = [i for i, v in enumerate(smape_values) if not np.isnan(v)]
        if not valid_idx:
            ax.text(0.5, 0.5, 'No data', ha='center', va='center', transform=ax.transAxes)
            ax.set_title(scenario_display.get(scenario, scenario), fontweight='bold', fontsize=12)
            continue

        valid_models = [models[i] for i in valid_idx]
        valid_values = [smape_values[i] for i in valid_idx]
        valid_colors = [colors[i] for i in valid_idx]

        bars = ax.bar(range(len(valid_models)), valid_values, color=valid_colors, alpha=0.85,
        ...
        ax.set_xticklabels(valid_models, rotation=40, ha='right', fontsize=10)
        ax.set_ylabel('Reconstruction SMAPE', fontweight='bold', fontsize=11)
        ax.set_title(scenario_display.get(scenario, scenario), fontweight='bold', fontsize=12)
        ax.grid(True, alpha=0.3, axis='y', linestyle='--')
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)

    plt.suptitle('Task 3: FDD - Physical Causal Chain Reconstruction',
                 fontweight='bold', fontsize=14, y=1.02)
    plt.tight_layout()

    # Save to task3_fdd directory
    fdd_dir = output_dir / 'task3_fdd'
    fdd_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(fdd_dir / 'fdd_reconstruction_comparison.png', dpi=180, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"  Saved: task3_fdd/fdd_reconstruction_comparison.png")

    # Also save individual scenario plots for detail
    for scenario in scenarios:
        fig, ax = plt.subplots(figsize=(10, 6))

        mae_values = [results[scenario][m].get('Recon_MAE', np.nan) for m in models]
        smape_values = [results[scenario][m].get('Recon_SMAPE', np.nan) for m in models]

        valid_idx = [i for i, v in enumerate(mae_values) if not np.isnan(v)]
        if not valid_idx:
            plt.close()
            continue

        valid_models = [models[i] for i in valid_idx]
        valid_mae = [mae_values[i] for i in valid_idx]
        valid_smape = [smape_values[i] for i in valid_idx]
        valid_colors = [MODEL_COLORS.get(models[i], '#95a5a6') for i in valid_idx]

        x = np.arange(len(valid_models))
        width = 0.35

        bars1 = ax.bar(x - width/2, valid_mae, width, label='Recon MAE', color='steelblue', alpha=0.8)
        bars2 = ax.bar(x + width/2, valid_smape, width, label='Recon SMAPE', color='coral', alpha=0.8)

        ax.set_ylabel('Metric Value', fontweight='bold')
        ax.set_title(f'{scenario}: {FDD_SCENARIOS.get(scenario, {}).get("description", scenario)}',
        ...
        plt.savefig(fdd_dir / f'{safe_name}_detail.png', dpi=150, bbox_inches='tight', facecolor='white')
        plt.close()

    print(f"  Saved: task3_fdd/ individual scenario plots")
...
def plot_single_task_item(
    ...
    task_dir_name: str,
    var_group: str,
    model_predictions: Dict[str, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]],
    output_dir: Path,
):
    """
    Plot a single task+item with all models side by side, unified y-axis scale.

    Args:
        task_name: Display name of the task (e.g., "Forecast", "Fill-in", "OOD")
        task_dir_name: Directory name for saving (e.g., "task1_forecast")
        var_group: Variable group name (e.g., "Main Power", "ODU Power")
        model_predictions: Dict mapping model_name to (context, y_true, y_pred, samples)
            - context: historical data before prediction (np.ndarray)
            - y_true: ground truth for prediction period (np.ndarray)
            - y_pred: point prediction (np.ndarray)
            - samples: distribution samples (np.ndarray) or None
    ...
    # Compute unified y-axis range across all models
    all_values = []
    for model_name, (context, y_true, y_pred, samples) in model_predictions.items():
        if context is not None:
            all_values.extend(context[~np.isnan(context)].flatten())
        if y_true is not None:
            all_values.extend(y_true[~np.isnan(y_true)].flatten())
        if y_pred is not None:
            all_values.extend(y_pred[~np.isnan(y_pred)].flatten())
        if samples is not None and len(samples) > 1:
            p5 = np.percentile(samples, 5, axis=0)
            p95 = np.percentile(samples, 95, axis=0)
            all_values.extend(p5[~np.isnan(p5)].flatten())
            all_values.extend(p95[~np.isnan(p95)].flatten())
    ...
    # Main plot + interval width subplot (height_ratios=[4, 1])
    fig, axes = plt.subplots(nrows * 2, ncols, figsize=(6 * ncols, 5 * nrows),
    ...
    if nrows == 1 and ncols == 1:
        ...
        axes = np.array([axes[:ncols], axes[ncols:]])
    else:
        # Reshape to (nrows, 2, ncols) then to (nrows*2, ncols)
        axes = axes.reshape(nrows, 2, ncols)

    for idx, (model_name, (context, y_true, y_pred, samples)) in enumerate(model_predictions.items()):
        row_idx = idx // ncols
        col_idx = idx % ncols
        ...
        if nrows == 1:
            ...
            ax_main = axes[row_idx, 0, col_idx]
            ax_width = axes[row_idx, 1, col_idx]

        # Build time axis
        context_len = len(context) if context is not None else 0
        pred_len = len(y_pred) if y_pred is not None else 0
        total_len = context_len + pred_len

        time_axis = np.arange(total_len)
        context_time = time_axis[:context_len]
        pred_time = time_axis[context_len:context_len + pred_len]

        pred_color = MODEL_COLORS.get(model_name, '#e74c3c')

        # Main plot: context
        if context is not None and len(context) > 0:
            ax_main.plot(context_time, context, '#3498db', alpha=0.4, label='Context', lw=1.2)

        # Main plot: confidence interval (align lengths if needed)
        interval_width = None
        if samples is not None and len(samples) > 1:
            p5 = np.percentile(samples, 5, axis=0)
            ...
            # Align CI lengths with pred_time
            ci_len = min(len(pred_time), len(p5))
            ci_time = pred_time[:ci_len]
            p5, p25, p75, p95 = p5[:ci_len], p25[:ci_len], p75[:ci_len], p95[:ci_len]

            ax_main.fill_between(ci_time, p5, p95, alpha=0.10, color=pred_color, label='90% CI')
            ax_main.fill_between(ci_time, p25, p75, alpha=0.18, color=pred_color, label='50% CI')

            interval_width = p95 - p5

        # Main plot: ground truth (align length with pred_time if needed)
        if y_true is not None:
            y_true_plot = y_true[:len(pred_time)] if len(y_true) > len(pred_time) else y_true
            plot_time = pred_time[:len(y_true_plot)]
            ax_main.plot(plot_time, y_true_plot, '#27ae60', lw=2.5, label='Truth')

        # Main plot: prediction (align length with pred_time if needed)
        if y_pred is not None:
            y_pred_plot = y_pred[:len(pred_time)] if len(y_pred) > len(pred_time) else y_pred
            plot_time_pred = pred_time[:len(y_pred_plot)]
            ax_main.plot(plot_time_pred, y_pred_plot, color=pred_color, linestyle='--', lw=2.0, label='Median')

        # Vertical line at prediction start
        if context_len > 0:
            ax_main.axvline(x=context_len, color='#7f8c8d', linestyle=':', alpha=0.7, lw=1.5)

        # Title with metrics (align lengths for calculation)
        title_text = model_name
        if y_true is not None and y_pred is not None:
            # Align lengths for metric calculation
            metric_len = min(len(y_true), len(y_pred))
            y_true_m = y_true[:metric_len]
            y_pred_m = y_pred[:metric_len]
            valid_mask = ~np.isnan(y_true_m) & ~np.isnan(y_pred_m)
            if np.any(valid_mask):
                smape = calculate_smape(y_true_m[valid_mask], y_pred_m[valid_mask])
                title_text += f'\nSMAPE={smape:.2f}'
                if samples is not None:
                    samples_m = samples[:, :metric_len] if samples.shape[1] > metric_len else samples
                    crps = calculate_crps(y_true_m, samples_m)
                    if not np.isnan(crps):
                        title_text += f' | CRPS={crps:.3f}'

        ax_main.set_title(title_text, fontweight='bold', fontsize=11)
        ax_main.set_ylim(unified_ylim)  # Unified y-axis
        ax_main.set_ylabel(var_group, fontweight='bold')
        ax_main.legend(loc='upper right', fontsize=7, framealpha=0.9)
        ax_main.grid(True, alpha=0.3, linestyle='--')
        ax_main.spines['top'].set_visible(False)
        ax_main.spines['right'].set_visible(False)

        # Width subplot: interval width over time (use aligned ci_time)
        if interval_width is not None:
            width_len = min(len(pred_time), len(interval_width))
            width_time = pred_time[:width_len]
            width_vals = interval_width[:width_len]
            ax_width.plot(width_time, width_vals, color='gray', lw=1, alpha=0.7)
            ax_width.fill_between(width_time, 0, width_vals, color='gray', alpha=0.15)
            ax_width.set_ylabel('CI Width', fontsize=8)
            ax_width.set_ylim(bottom=0)
            ax_width.tick_params(labelsize=7)
        else:
            ax_width.text(0.5, 0.5, 'No CI', ha='center', va='center',
                         transform=ax_width.transAxes, fontsize=9, color='#bdc3c7')

        ax_width.set_xlabel('Time Step', fontweight='bold', fontsize=9)
        ax_width.grid(True, alpha=0.2, linestyle='--')
        ax_width.spines['top'].set_visible(False)
        ax_width.spines['right'].set_visible(False)

    # Hide unused axes
    for idx in range(len(model_predictions), nrows * ncols):
        row_idx = idx // ncols
        col_idx = idx % ncols
        if nrows == 1:
            ...
            axes[row_idx, 0, col_idx].axis('off')
            axes[row_idx, 1, col_idx].axis('off')

    plt.suptitle(f'{task_name}: {var_group}', fontweight='bold', fontsize=14, y=1.01)
    plt.tight_layout()

    # Safe filename
    safe_group = var_group.replace(' ', '_').replace('/', '_').replace('[', '').replace(']', '').lower()
    filename = f'{task_dir_name.split("_")[0]}_{safe_group}.png'
    plt.savefig(task_dir / filename, dpi=180, bbox_inches='tight', facecolor='white')
    plt.close()
    print(f"    Saved: {task_dir_name}/{filename}")
...
def plot_sample_predictions(
    ...
    for idx, (model_name, model) in enumerate(models.items()):
        ...
        if samples is not None and len(samples) > 1:
            ...
            # Plot 90% interval (5th-95th) - more transparent for clarity
            ax.fill_between(pred_time_axis, p5, p95, alpha=0.10, color=pred_color, label='90% CI')
            # Plot 50% interval (25th-75th)
            ax.fill_between(pred_time_axis, p25, p75, alpha=0.18, color=pred_color, label='50% CI')
...
def generate_split_task_item_plots(
    models: Dict[str, object],
    test_hf: datasets.Dataset,
    output_dir: Path,
    device: str
    ...
    """
    Generate split plots for each Task + Variable Group combination.

    Output structure:
    - task1_forecast/: Standard Forecasting plots by variable group
    - task2_fillin/: Fill-in-the-Blank plots
    - task3_ood/: OOD stress test plots by condition

    Each plot has unified y-axis for the same task+item across all models.
    """
    sample = test_hf[0]
    target = np.array(sample['target'])
    ...
    # ==========================================================================
    # Task 1: Standard Forecasting - one plot per variable group
    # ==========================================================================
    print("    Task 1 (Forecast): Generating per-group plots...")

    for group_name, group_info in EVAL_GROUPS.items():
        id_start, id_end = group_info['id_range']
        # Use the middle variable as representative
        rep_var_id = (id_start + id_end) // 2

        var_data = target[rep_var_id, :]
        start_idx = find_best_window(var_data, CONTEXT_LENGTH, PREDICTION_LENGTH)
        full_window = var_data[start_idx:start_idx + CONTEXT_LENGTH + PREDICTION_LENGTH]

        if len(full_window) < CONTEXT_LENGTH + PREDICTION_LENGTH:
            continue
        ...
        for model_name, model in models.items():
            ...
            try:
                if model_name == 'Seasonal Naive':
                    ...
                    point_pred, samples = predict_moirai(model, full_window, CONTEXT_LENGTH, PREDICTION_LENGTH, PATCH_SIZE, NUM_SAMPLES, device, var_id=rep_var_id)
                else:
                    continue

                model_predictions[model_name] = (context, future_true, point_pred, samples)
            except Exception as e:
                print(f"      Warning: {model_name} failed on {group_name}: {e}")
                continue

        if model_predictions:
            plot_single_task_item(
                task_name="Forecast",
                task_dir_name="task1_forecast",
    ...
    # ==========================================================================
    # Task 2: Fill-in-the-Blank - ODU Power only
    # ==========================================================================
    print("    Task 2 (Fill-in): Generating ODU Power plot...")

    # ODU Power var ID range is 14-17 (from EVAL_GROUPS)
    odu_var_id = 15  # Representative ODU Power variable
    var_data = target[odu_var_id, :]
    start_idx = find_best_window(var_data, CONTEXT_LENGTH, PREDICTION_LENGTH)

    if start_idx >= 0:
        total_length = CONTEXT_LENGTH + PREDICTION_LENGTH
        full_window = var_data[start_idx:start_idx + total_length]

        if len(full_window) >= total_length:
            model_predictions = {}
            for model_name, model in models.items():
                if model is None or not isinstance(model, MoiraiModule):
                    continue

                try:
                    # For fill-in task, we provide weather + zone temps, mask ODU power
                    full_data = target[:, start_idx:start_idx + total_length]
                    ...
                    if odu_var_id in predictions:
                        # predict_multivariate returns (point_pred, samples, mask_start, mask_end) for fill_in_blank
                        result = predictions[odu_var_id]
                        point_pred, samples = result[0], result[1]
                        mask_start, mask_end = result[2], result[3]

                        # Extract truth and context aligned with the masked region
                        masked_true = full_window[mask_start:mask_end]
                        context_before = full_window[:mask_start]

                        model_predictions[model_name] = (context_before, masked_true, point_pred, samples)
                except Exception as e:
                    print(f"      Warning: {model_name} failed on Fill-in: {e}")
                    continue

            if model_predictions:
                plot_single_task_item(
                    task_name="Fill-in",
                    task_dir_name="task2_fillin",
                    var_group="ODU Power",
                    model_predictions=model_predictions,
                    output_dir=output_dir
                )
    ...
    # ==========================================================================
    # Task 3: OOD Stress Test - one plot per condition
    # ==========================================================================
    print("    Task 3 (OOD): Generating per-condition plots...")

    # Find samples for each OOD condition
    for condition in ['high_temp', 'low_temp', 'high_load']:
        # Find appropriate sample for this condition
        sample_idx = None
        for idx in range(min(50, len(test_hf))):
            sample = test_hf[idx]
            sample_target = np.array(sample['target'])

            # Check outdoor temp (var 0)
            outdoor_temp = sample_target[0, :]
            mean_temp = np.nanmean(outdoor_temp)

            # Check load level (var 10-13)
            power_data = sample_target[10:14, :]
            max_power = np.nanmax(power_data)

            if condition == 'high_temp' and mean_temp > OOD_THRESHOLDS.get('high_temp', 32):
                sample_idx = idx
                break
            elif condition == 'low_temp' and mean_temp < OOD_THRESHOLDS.get('low_temp', 5):
                sample_idx = idx
                break
            elif condition == 'high_load' and max_power > 0:
                # Check if power is in high percentile
                all_powers = []
                for i in range(min(20, len(test_hf))):
                    s = test_hf[i]
                    all_powers.extend(np.array(s['target'])[10:14, :].flatten())
                threshold = np.nanpercentile(all_powers, OOD_THRESHOLDS.get('high_load', 0.9) * 100)
                if max_power > threshold:
                    sample_idx = idx
                    break

        if sample_idx is None:
            print(f"      No suitable sample found for {condition}")
            continue

        sample = test_hf[sample_idx]
        sample_target = np.array(sample['target'])

        # Use power variable (var 10) as representative
        rep_var_id = 10
        var_data = sample_target[rep_var_id, :]
        start_idx = find_best_window(var_data, CONTEXT_LENGTH, PREDICTION_LENGTH)

        if start_idx < 0 or len(var_data) < start_idx + CONTEXT_LENGTH + PREDICTION_LENGTH:
            continue
        ...
        for model_name, model in models.items():
            ...
            try:
                ...
                print(f"      Warning: {model_name} failed on OOD {condition}: {e}")
                continue

        # Use Windows-safe filenames (no >, <, or special chars)
        condition_display = {
            'high_temp': f'High Temp above {OOD_THRESHOLDS.get("high_temp", 32)}C',
            'low_temp': f'Low Temp below {OOD_THRESHOLDS.get("low_temp", 5)}C',
            'high_load': f'High Load above {OOD_THRESHOLDS.get("high_load", 0.9)*100:.0f}pct'
        }

        if model_predictions:
            plot_single_task_item(
                task_name="OOD",
                task_dir_name="task3_ood",
                var_group=condition_display.get(condition, condition),
                model_predictions=model_predictions,
                output_dir=output_dir
            )
...
def create_summary_table(all_results: Dict, output_dir: Path):
    ...
    if 'task3' in all_results:
        for scenario, scenario_results in all_results['task3'].items():
            for model_name, metrics in scenario_results.items():
                row = {
                    'Task': f'FDD ({scenario})',
                    'Model': model_name,
                    'Variable Group': scenario,
                    **metrics
                }
                rows.append(row)

    # Extra: OOD results
    if 'task_ood' in all_results:
        for condition, condition_results in all_results['task_ood'].items():
    ...
    if 'task2' in all_results:
        print("\n[Task 2: Virtual Sensing / Fill-in-the-Blank]")
        for model_name, metrics in all_results['task2'].items():
            print(f"  {model_name}: SMAPE={metrics.get('SMAPE', np.nan):.2f}, MAE={metrics.get('MAE', np.nan):.4f}")

    if 'task3' in all_results:
        print("\n[Task 3: FDD (Fault Detection & Diagnostics)]")
        for scenario in all_results['task3'].keys():
            print(f"  {scenario}:")
            for model_name, metrics in all_results['task3'][scenario].items():
                recon_mae = metrics.get('Recon_MAE', np.nan)
                recon_smape = metrics.get('Recon_SMAPE', np.nan)
                if not np.isnan(recon_mae):
                    print(f"    {model_name}: Recon_MAE={recon_mae:.4f}, Recon_SMAPE={recon_smape:.2f}")

    if 'task_ood' in all_results:
        print("\n[Extra: OOD Stress Test]")
        for condition in all_results['task_ood'].keys():
            print(f"  {condition}:")
            for model_name, metrics in all_results['task_ood'][condition].items():
...
def main():
    ...
    # 示例:
    MODELS_TO_COMPARE = {
        'Small-Full-7e6': 'moirai_small_full_7e6',
        'Small-Freeze-2e5': 'moirai_small_freeze_ffn_2e5',
        'Small-Head-2e4': 'moirai_small_head_only_2e4',
        'Base-Full-2e6': 'moirai_base_full_2e6',
        'Base-Freeze-1e5': 'moirai_base_freeze_ffn_1e5',
    ...
    print("Tasks: Standard Forecast | Virtual Sensing | FDD | OOD (Extra)")
    ...
    # Task 3: FDD (Fault Detection & Diagnostics)
    # =========================================================================
    print("\n[5/7] Task 3: FDD (Fault Detection & Diagnostics)...")
    print("  Testing physical causal chain learning:")
    for scenario, config in FDD_SCENARIOS.items():
        print(f"    - {scenario}: {config['description']}")

    # Run FDD evaluation (MOIRAI models only, as baselines don't support this)
    if moirai_models:
        task3_results = task3_fdd(models, test_hf, MAX_EVAL_SAMPLES, device)
        all_results['task3'] = task3_results

        for scenario_name, scenario_results in task3_results.items():
            print(f"\n  {scenario_name}:")
            for model_name, metrics in scenario_results.items():
                recon_mae = metrics.get('Recon_MAE', np.nan)
                recon_smape = metrics.get('Recon_SMAPE', np.nan)
    ...
    # =========================================================================
    # Extra: OOD Stress Test (optional)
    # =========================================================================
    print("\n[Extra] OOD Stress Test (extreme conditions)...")
    ood_var_ids = list(range(10, 14)) + list(range(50, 55))  # Power + Zone temps
    ood_results = task_extra_ood(models, test_hf, ood_var_ids, MAX_EVAL_SAMPLES, device)
    all_results['task_ood'] = ood_results

    for condition, condition_results in ood_results.items():
        print(f"  {condition}:")
        for model_name, metrics in condition_results.items():
    ...
    # FDD results (Task 3)
    if all_results.get('task3') and any(all_results['task3'].values()):
        plot_fdd_results(all_results['task3'], output_dir)

    # OOD stress test results (Extra task)
    if all_results.get('task_ood') and any(all_results['task_ood'].values()):
        plot_ood_performance(all_results['task_ood'], output_dir)
    ...
    # Split plots by task and item (with unified y-axis per task+item)
    print("  Generating split task/item plots...")
    generate_split_task_item_plots(models, test_hf, output_dir, device)
